...
static uint32_t submap_offset( const tripoint_abs_sm &pos )
{
    uint32_t h = 0;
    h ^= static_cast<uint32_t>( pos.x() );
    h = ( h << 13 ) ^ ( h >> 19 );
    h ^= static_cast<uint32_t>( pos.y() );
    h = ( h << 7 ) ^ ( h >> 25 );
    h ^= static_cast<uint32_t>( pos.z() );
    h = ( h << 17 ) ^ ( h >> 15 );
...
void mapbuffer::process_visible_plants_randomly(const tripoint_abs_sm &pos) {
    ...
    // Uniform submaps (empty rock, open air, boundary fill) have no plants
    if( tmpsub->is_uniform ) {
        tmpsub->last_touched = calendar::turn;
        return;
    }
